...
*/
const execAsync$1 = promisify(execFile);
const MAX_BUFFER = 10485760;
/** Bound a concrete OKS binary into a callable runner (no shell, no globbing). */
function makeOksFsRunner(oksBin) {
	return async (args) => {
		const { stdout } = await execAsync$1(oksBin, args, {
...
*/
function parseOksFsTree(stdout) {
	let data;
	try {
	...
	const body = data;
	if (body?.schema_version !== "oks-fs-response/v1" || body?.operation !== "tree" || !body.result || !Array.isArray(body.result.entries)) return null;
	const files = [];
	const directories = [];
...
*/
async function listVfsFiles(run, rootUri) {
	try {
		const tree = parseOksFsTree(await run([
			"fs",
			"tree",
			rootUri,
			"--depth",
		...
		return {
			files: tree.files.filter((uri) => uri.startsWith(prefix)).map((uri) => {
			...
			}).filter((rel) => rel.length > 0).filter(isSafeVfsRel),
...
*/
function createVfsRunnerRef(readEnabled, probe) {
	let cached;
	return () => {
		if (readEnabled() !== true) return Promise.resolve(void 0);
		cached ??= probe();
		return cached;
	};
}
...
*/
async function markdownFiles(root, run, rootUri) {
	if (run && rootUri) {
		const vfs = await listVfsFiles(run, rootUri);
		if (vfs) return {
			files: vfs.files.filter((rel) => rel.toLowerCase().endsWith(".md") && rel !== ".gitkeep").map((rel) => join(root, rel)),
			truncated: vfs.truncated
		};
	}
	const files = [];
	const pending = [root];
...
async function listMarkdownPages(knowledgeBasePath, directory, filters = {}, run) {
...
async function getMarkdownPage(knowledgeBasePath, directory, requestedSlug, run) {
...
function listWikiPages(knowledgeBasePath, filters = {}, run) {
	return listMarkdownPages(knowledgeBasePath, "wiki", filters, run);
}
function getWikiPage(knowledgeBasePath, requestedSlug, run) {
	return getMarkdownPage(knowledgeBasePath, "wiki", requestedSlug, run);
}
function listDraftPages(knowledgeBasePath, filters = {}, run) {
	return listMarkdownPages(knowledgeBasePath, "drafts", filters, run);
}
function getDraftPage(knowledgeBasePath, requestedSlug, run) {
...
async function findBundleDirectories(rawRoot, run) {
	if (run) {
		...
		if (vfs) {
			...
			return {
				directories: directories.sort((a, b) => relativeId(rawRoot, a).localeCompare(relativeId(rawRoot, b))).slice(0, MAX_BUNDLE_DIRECTORIES),
				truncated
			};
		}
	}
...
async function listRawBundles(knowledgeBasePath, filters = {}, run) {
...
async function getRawBundle(knowledgeBasePath, requestedId, run) {
	const id = normalizeFilter(requestedId);
	if (!id || id.includes("\\") || id.split("/").some((part) => !part || part === "." || part === "..")) return void 0;
	const rawRoot = resolve(knowledgeBasePath, "raw");
	const directory = (await findBundleDirectories(rawRoot, run)).directories.find((candidate) => relativeId(rawRoot, candidate) === id);
...
async function countFiles(root, include, run, rootUri) {
	if (run && rootUri) {
		const vfs = await listVfsFiles(run, rootUri);
		if (vfs) {
			let count = 0;
...
async function getOksOverview(knowledgeBasePath, run) {
...
*/
async function getOksDiagnostics(knowledgeBasePath, oksCliAvailable, run) {
	const empty = {
		wikiCount: 0,
		draftCount: 0,
		rawFileCount: 0,
...
function parseRecallJson(stdout) {
	...
	if (knowledge === void 0 && episodic === void 0) throw new Error("OKS recall response has no result arrays");
	if (knowledge !== void 0 && !Array.isArray(knowledge) || episodic !== void 0 && !Array.isArray(episodic)) throw new Error("OKS recall response has invalid result arrays");
	const isCandidate = (value) => Boolean(value) && typeof value === "object" && !Array.isArray(value);
	if ((knowledge ?? []).some((value) => !isCandidate(value)) || (episodic ?? []).some((value) => !isCandidate(value))) throw new Error("OKS recall response has invalid candidates");
	return {
		knowledge: knowledge ?? [],
		episodic: episodic ?? []
	};
...
function mergeRecallResults(results) {
	const seen = /* @__PURE__ */ new Set();
	const knowledge = [];
	const episodic = [];
	let succeeded = 0;
	...
	for (const result of results) {
		...
		for (const h of result.value.episodic) {
			if (!h || typeof h !== "object" || Array.isArray(h)) continue;
			const sourcePath = String(h.source_path ?? "");
...
function parseStatusTiers(stdout) {
	...
	return {
		hot: Number(tierMatch[1]),
		warm: Number(tierMatch[2]),
		cold: Number(tierMatch[3]),
		evictable: Number(tierMatch[4]),
...
function apply(ctx, config = {}) {
	...
	const browserRun = createVfsRunnerRef(() => settingsHooks.getCurrent().vfs_enabled === true, async () => {
		const run = makeOksFsRunner(oksBin());
		return await probeOksFs(run) ? run : void 0;
	});
	ctx.connection.rpc.handle("/oks", async (endpoint, payload) => {
		...
		if (endpoint === "diagnostics") {
			...
			try {
				...
				return {
					ok: true,
					value: await getOksDiagnostics(configuredPath, oksCliAvailable, run)
				};
			} catch (error) {
		...
		try {
			...
			if (endpoint === "overview") return {
				ok: true,
				value: await getOksOverview(configuredPath, run)
			};
			if (endpoint === "lint") try {
				const result = parseLintOutput(await runOks(["lint"]));
				...
				return {
					...
					value: {
						errors: -1,
						warnings: 0,
						items: ["OKS CLI 未返回健康检查结果"],
						summary: "",
						passed: false
					}
				};
			}
			if (endpoint === "status-tiers") try {
				const tiers = parseStatusTiers(await runOks(["status"]));
				if (!tiers) return {
					ok: false,
					error: {
						code: "internal",
						message: "Unable to parse OKS status output.",
						details: {}
					}
				};
				return {
				...
				return {
					...
					error: {
						code: "internal",
						message: "OKS CLI did not return status.",
						details: {}
					}
				};
			}
	...
	ctx.tools.register(defineTool({
		...
		async execute(args) {
			...
			const batch = mergeRecallResults(await Promise.allSettled(all.map((q) => runOks([
				"recall",
				q,
				"--format",
				"json",
			...
			const detail = batch.failed > 0 ? `合并 ${trace?.candidateCount ?? 0} 个候选；${batch.failed}/${batch.succeeded + batch.failed} 个查询失败` : `合并 ${trace?.candidateCount ?? 0} 个候选`;
	...
	ctx.tools.register(defineTool({
		name: "oks_lint",
		description: "Run a health check on the OKS knowledge base: validate frontmatter, detect orphan pages, check backlinks, and report coverage. Use this to diagnose knowledge base issues or verify integrity after bulk edits.",
		parameters: {},
		output: {
			schema: { type: "string" },
			render: (_args, value) => [{
...
//#endregion
export { OKS_NS, OksConfigSchema, apply, inject, mergeRecallResults, name, parseRecallJson, parseSignal };
