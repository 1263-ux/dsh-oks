...
test('client UI keeps Chinese navigation and safe settings bindings', async () => {
  ...
  assert.ok(panel.includes('<RecallTracePanel rpc={rpc} compact />'))
  assert.ok(panel.includes('<ActivityPanel rpc={rpc} compact />'))
  assert.equal(panel.includes('<RecallTracePanel rpc compact />'), false)
  assert.equal(panel.includes('<ActivityPanel rpc compact />'), false)
