...
test('client UI keeps Chinese navigation and safe settings bindings', async () => {
  ...
  assert.ok(panel.includes('aria-label="回答时自动参考我的知识"'))
  assert.ok(panel.includes('>OFF</span>'))
  assert.ok(panel.includes('>ON</span>'))
  assert.ok(panel.includes("background: enabled ? T.success : '#d9dde3'"))
  assert.ok(panel.includes("background: '#fff'"))
  ...
  assert.ok(params.includes('aria-label="自动召回"'))
  assert.ok(params.includes('>OFF</span>'))
  assert.ok(params.includes('>ON</span>'))
  assert.ok(params.includes("background: Boolean(v.prestep_enabled ?? true) ? T.success : '#d9dde3'"))
  assert.ok(params.includes("background: '#fff'"))
  ...
  assert.match(clientIndex, /export const inject = \['slots', 'locale', 'connection', 'remote', 'settingsScope'\]/)
  assert.match(clientIndex, /name: 'dsh-oks-sidebar'/)
  assert.match(clientIndex, /inject: \['betterSidebar'\]/)
  assert.ok(clientIndex.includes("ctx.get('betterSidebar', false)"))
  assert.ok(clientIndex.includes('unavailableRpc'))
  assert.equal(clientIndex.includes('createHttpRpc'), false)
